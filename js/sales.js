...
$(".make-proforma").click(function () {
 ...
 // discount = document.querySelector("#discount").innerText.replace(/[,]/g, "");
  ...
  $(document).ready(function () {
    var sale_no = 0;
    $.ajax({
      url: "http://localhost/Rojo_sales/generate_proforma_invoice.php/",
      method: "post",
      cache: false,
      data: { details: JSON.stringify(sales_details) },
      dataType: "json",
      success: function (data) {
        ...
        sale_no = data.last_id;
        $(".print-link").html(
          `<a href="http://localhost/Rojo_sales/proformaInvoice.php?id=${sale_no}" class="btn btn-primary" target="_blank" rel="noreferrer">Print Invoice</a>`
        );
      },
