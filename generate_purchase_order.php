...
// }
   ...
   $sql = "INSERT INTO `purchase_order` (`id`, `customer_name`, `purchase_details`, `created_At`) VALUES (NULL, '$customer_name', '$product_order',  CURRENT_TIMESTAMP)";

    try{
        $status=$conn->query($sql);

        $last_id=$conn->insert_id;
        echo json_encode(['message'=>'successfully purchases','status_code'=>$status,'last_id'=>$last_id]);
    }
    catch(Exception $e)
{
